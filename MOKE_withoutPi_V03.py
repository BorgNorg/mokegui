...
class MOKEGUI:
    ...
    def __init__(self):
        ...
        self.minV.grid(row=2, column=1, padx=10, pady=5)

        self.stepV = ctk.CTkTextbox(master=frame, height=20, width=80)
        self.stepV.grid(row=3, column=1, padx=10, pady=5)
        ...
        self.label_min.grid(row=2, column=0, padx=10, pady=5)

        self.label_step = ctk.CTkLabel(master=frame, text="Step Voltage (V)")
        self.label_step.grid(row=3, column=0, padx=10, pady=5)
    ...
    def textbox(self):
        name_sample = self.sample_name.cget
        max_volt = self.maxV.cget
        min_volt = self.minV.cget
        step_volt = self.stepV.cget


moke_instance = MOKEGUI()
